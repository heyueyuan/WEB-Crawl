...
def write2csv(url, data):
    ...
    with open('E:\\bj-zufang\\{}.csv'.format(name), 'a', newline='', encoding='utf-8-sig') as f:
...
for area in ['dongcheng', 'xicheng', 'chaoyang', 'haidian', 'fengtai', 'shijingshan', 'tongzhou', 'changping', 'daxing',
             'yizhuangkaifaqu', 'shunyi', 'fangshan', 'mentougou', 'pinggu', 'huairou', 'miyun', 'yanqing','yanjiao',
             'ditiezufang', 'all']:
    base_url = 'https://bj.lianjia.com/zufang/{}/pg1/'.format(area)
    num = get_page_num(base_url)
    for page in range(1, int(num) + 1):
        url = 'https://bj.lianjia.com/zufang/{}/pg{}/'.format(area, page)
        html = get_html(url)
        data = RoomInfo(html)
        write2csv(url, data)
